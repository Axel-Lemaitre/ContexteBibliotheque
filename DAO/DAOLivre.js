...
class DAOLivre {
    ...
    getLesExemplaire(num, cb) {
        ...
        this._client.query(query, function (err, result) {
            ...
            if (err) {
                ...
                result.rows.forEach(function (row) {
                    let unExemplaire;
                    unExemplaire = new Exemplaire(row['iddocument'], row['numero'], row['statut'], row['dateretour'], row['titre']);
    ...
    getUnExemplaire(id, id2, cb) {
        ...
        let query = {
            name: 'fetch-un-livre',
            text: "SELECT * FROM exemplaire INNER JOIN livre ON livre.iddocument = exemplaire.iddocument WHERE exemplaire.iddocument = $1 AND exemplaire.numero = $2;",
            values: [id, id2]
        };
    ...
    setNewExemplaire(id) {
        const query = {
            name: 'add-new-exemplaires',
            text: "select creeexemplaire($1);",
            values: [id]
        };
    ...
    dropExemplaire(id, name) {
        const query = {
            name: 'drop-exemplaire',
            text: "select dropexemplaire($1, $2);",
            values: [id, name]
        };
